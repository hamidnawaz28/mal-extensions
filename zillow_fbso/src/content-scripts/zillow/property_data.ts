...
const entry = async () => {
    ...
    Browser.runtime.onMessage.addListener(async function (request: any) {
        ...
        // For agent listing
        if (message == MESSAGING.COLLECT_AGENT_LISTING_DATA) {
        ...
        if (message === MESSAGING.COLLECT_FSBO_LISTING_DATA) {
            ...
            const sqft = sqftRef?.innerText?.replace(/[,.]/g, '').replace(',', '')
