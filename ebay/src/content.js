...
// - Background call: { type: "SCAN_MESSAGE", payload }
// - Manual Add overlay
...
;(() => {
  ...
  const CHECK_USAGE_LIMIT = 'CHECK_USAGE_LIMIT'
  ...
  async function loadSettings() {
    ...
    return {
      ...
      startDate: d.startDate || _todayStr(new Date(new Date().getFullYear(), 0, 1)),
  ...
  function resultToItem(r) {
    ...
    const name = r.name || r.title || r.label || (r.is_recipe ? 'Recipe' : 'Item')
    return {
      name,
  ...
  function ensurePanel() {
    ...
    try {
      ...
      var __apply = function (st) {
        try {
          countWrap.style.display = st && st.paid ? 'none' : ''
        } catch (e) {}
      }
      if (__mgr && typeof __mgr.getUserStatus === 'function') {
        __apply(__mgr.getUserStatus())
      }
    ...
    intro2.textContent = 'TO BE MORE ACCURATE, MAKE SURE YOU HAVE NUTRITION FACTS OPEN'
    ...
    // actions
    var __closeEl = panel.querySelector('#' + ID_PANEL_CLOSE)
    if (__closeEl) {
      __closeEl.addEventListener('click', function () {
    ...
    btnScan.addEventListener('click', async () => {
      try {
        ...
        if (mgr) {
          // get fresh status
          try {
            await mgr.refreshSubscriptionStatus()
          } catch (_e) {}
  ...
  function setupCalendarOverlay() {
    ...
    const sMaint = labeledInput(ID_SET_MAINT, 'Maintenance (kcal/day)', 'number')
    const sDesired = labeledInput(ID_SET_DESIRED, 'Desired Intake (kcal/day)', 'number')
  ...
  function renderResult(res, modelName) {
    ...
    const { per, tot, servings, servingSize, confidence, assumptions } = normalizeResult(res)
    ...
    wrap.innerHTML = `
      ...
      servingSize ? ` &nbsp;|&nbsp; Size: <b>${escapeHtml(servingSize)}</b>` : ''
    }
        </div>
      </div>
      <div class="mt-card">
        <div class="mt-note">${
          confidence != null ? `Confidence: <b>${Math.round(confidence * 100)}%</b>` : ''
        } <span style="opacity:.7">(${escapeHtml(modelName || 'background')})</span></div>
      </div>
    `
    c.appendChild(wrap)
    ...
    details.innerHTML = `<summary>Assumptions & Notes</summary>
      <ul style="margin:6px 0 0 14px">${
        assumptions.map((a) => `<li>${escapeHtml(a)}</li>`).join('') || '<li>None provided.</li>'
      }</ul>`
